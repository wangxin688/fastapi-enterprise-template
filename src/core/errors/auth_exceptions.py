...
from src.core.errors.base_exceptions import ErrorCode
...
async def token_invalid_handler(request: Request, exc: TokenInvalidError) -> JSONResponse:
    ...
    response_content = base_exceptions.ERR_10002.dict()
...
async def invalid_token_for_refresh_handler(request: Request, exc: TokenInvalidForRefreshError) -> JSONResponse:
    log_exception(exc, False)
    return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED, content=base_exceptions.ERR_10004.dict())


async def token_expired_handler(request: Request, exc: TokenExpireError) -> JSONResponse:
    log_exception(exc, False)
    return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED, content=base_exceptions.ERR_10003.dict())


async def permission_deny_handler(request: Request, exc: PermissionDenyError) -> JSONResponse:
    log_exception(exc, False)
    return JSONResponse(status_code=status.HTTP_403_FORBIDDEN, content=base_exceptions.ERR_10004.dict())


async def resource_not_found_handler(request: Request, exc: NotFoundError) -> JSONResponse:
    log_exception(exc, True)
    error_message = _(base_exceptions.ERR_404.message, name=exc.name, filed=exc.field, value=exc.value)
    content = {"error": base_exceptions.ERR_404.error, "message": error_message}
    return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=content)


async def resource_exist_handler(request: Request, exc: ExistError) -> JSONResponse:
    log_exception(exc, True)
    error_message = _(base_exceptions.ERR_409.message, name=exc.name, filed=exc.field, value=exc.value)
    content = {"error": base_exceptions.ERR_409.error, "message": error_message}
    return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content=content)
